...
### Análisis respecto a las horas de estudio
...
<div class="panelset">
<div class="panel">
<div class="panel-name">Modelo 1</div>
<div>
...
<div class="panel">
<div class="panel-name">Modelo 2</div>
<div>
```{r,message=FALSE, echo=TRUE}
...
```r
modelo_2_forward <- ols_step_forward_p(modelo_inicial, penter = 0.05) # p-value to be considered in model
modelo_2_forward
...
<div class="panel">
<div class="panel-name">Modelo 3</div>
<div>
<h4>Verificación</h4>
```{r,message=FALSE, echo=TRUE}
...
<div class="panel">
<div class="panel-name">Predicciones</div>
<div>
...
<script>
  ...
  for(i=0; i<tablinks.length; i++) {
    tablinks[i].addEventListener('click',function() {
      ...
      (tabcontent[this.dataset.dni]).style.display = 'block';
      (tablinks[this.dataset.dni]).classList.add('active');
    });
  }
</script>
...
renderPlotly({
  ...
  df_desordenado <- data.frame(probabilidad_gustar_stats = probabilidad(leer,aprender,bailar,datos_logistico$horas_estudio),
  ...
  data_predicha <- combinar_data_frames(
                                        data_predicha,
                                        data.frame(dominio,proba_predicha=probabilidad(leer,aprender,bailar,dominio))
                                      )

  ## Graficamos
  # No le gusta Estadística
  fig <- plot_ly(data=data_predicha, x=~horas_estudio_0,y=~proba_gustar_stats_0,
                name='\n\nEncuestados que no\nles gusta Estadística\n\n\n'
                )

  fig <- fig %>% layout(
                        title="¿Le gusta estudiar Estadística?",
                        xaxis=list(title='Horas de estudio',range=c(0,5+max(encuesta_sin_NA$Horas_de_estudio))),
                        yaxis=list(title="Probabilidad predicha de que le gusta Estadística",
                                  range=c(0,1))
                        ) 
  ...
  fig <- fig %>% add_trace(x=~horas_estudio_1,y=~proba_gustar_stats_1,
                          name='\n\nEncuestados que sí\nles gusta Estadística\n\n\n',
                          text = ~paste("Horas de estudio: ",round(horas_estudio_1,2),"\nProbabilidad: ",round(proba_gustar_stats_1,2),sep=''),
                          hoverinfo = 'text',marker=list(size=7)
                          )

  # Probabilidad
  fig <- fig %>% add_trace(x=~dominio,y=~proba_predicha, mode = "lines",
                          name='Probabilidad predicha,\nconsiderando que la\nprobabilidad se acerca a\nuno cuando a la\npersona encuestada le\ngusta Estadística.',line=list(color='black',width=0.5))
